...
class Agent:
    ...
    # Fidelis specific processing
    def restart_fe(self):
		# Let's assume the process is already running.  If it's not it'll throw an error
	    try:
		    subprocess.check_output('net stop feagent', stderr=subprocess.STDOUT)
	    except:
		    pass
	
	    # Let's assume that the process is stopped.  If there is an error still continue the analysis
	    try:
		    subprocess.check_output('net start feagent', stderr=subprocess.STDOUT)
	    except:
		    pass
